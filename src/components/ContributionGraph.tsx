...
export function ContributionGraph({
  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-lg p-3 sm:p-4 text-white">
      <div className="mb-3">
        <h3 className="text-xs sm:text-sm text-gray-200">
        ...
        <div className="overflow-x-auto">
          <div className="flex items-start min-w-max">
            ...
            <div className="flex flex-col mr-2">
              <div className="h-4 mb-1"></div> {/* Space for month labels */}
              {['Mon', '', 'Wed', '', 'Fri', '', ''].map((day, index) => (
                <div
                  key={index}
                  className="h-2 mb-0.5 text-xs text-gray-400 flex items-center w-6"
                >
                  {day}
                </div>
              ))}
            ...
            <div className="flex-1">
              {/* Month labels */}
              <div className="flex mb-1 text-xs text-gray-400 h-4 items-end">
                {[
                  'Aug',
                  'Sep',
                  'Oct',
                    ...
                    {week.map((day, dayIndex) => (
                      <div
                        ...
                        className={`w-2 h-2 rounded-sm ${
                          day.date
                            ? LEVEL_COLORS[
                                day.level as keyof typeof LEVEL_COLORS
                              ]
                            : 'bg-gray-800'
                        }`}
                        title={
                          day.date
                            ? `${day.count} contributions on ${new Date(day.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}`
                            : ''
                        }
                      />
                    ))}
      ...
      <div className="hidden sm:flex items-start">
        ...
        <div className="flex flex-col mr-2">
          ...
          {['Mon', '', 'Wed', '', 'Fri', '', ''].map((day, index) => (
            <div
              key={index}
              className="h-2.5 mb-0.5 text-xs text-gray-400 flex items-center w-6"
            >
              {day}
            </div>
          ))}
      ...
      {/* Legend */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between text-xs text-gray-400 mt-3 space-y-2 sm:space-y-0">
